...
- Model Parameters and Data
  ...
  - The CNN model has parameters represented by filters at multiple layers ($\Phi$, $\Psi$, $\Omega$) and classifier parameters ($\mathbf{W}$).
...
- $I_n$ = the $n$-th input image
- $y_n \in \{+1,-1\}$ = binary class label

- Loss Function and Learning Objective

  - An energy function E measures the loss between the true label $y_n and the predicted label $l_n from the model.
  - The goal of learning is to find parameters ($\Phi$, $\Psi$, $\Omega$, $\mathbf{W}$) that minimize the total loss over all training examples.
